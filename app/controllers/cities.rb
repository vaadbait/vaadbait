Vaadbait::App.controllers :cities do
  get :index do
    @title = "Cities"
    @cities = City.all(:id.gt => 0)
    render 'cities/index'
  end
  ...
  delete :destroy_many do
    ...
    ids = params[:city_ids].split(',').map(&:strip).map(&:to_i)
    cities = City.all(:id => ids)
